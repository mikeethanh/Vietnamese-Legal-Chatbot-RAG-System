...
def rewrite_query_to_multi_queries(original_query: str, num_queries: int = 3) -> List[str]:
    """
...
Bây giờ hãy tạo {num_queries} câu hỏi cho câu hỏi gốc trên:"""
    ...
    messages = [
        {"role": "system", "content": "Bạn là chuyên gia tư vấn pháp luật Việt Nam, giỏi diễn đạt câu hỏi pháp lý."},
    ...
    try:
        ...
        if len(queries) < num_queries:
            logger.warning(f"Only generated {len(queries)} queries, expected {num_queries}")
...
Câu hỏi đã viết lại:"""
    ...
    messages = [
        {"role": "system", "content": "Bạn là trợ lý AI chuyên viết lại câu hỏi để làm rõ ngữ nghĩa."},
        {"role": "user", "content": prompt}
    ]
