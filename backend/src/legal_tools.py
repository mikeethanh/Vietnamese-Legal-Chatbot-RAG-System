    ...
    try:
        ...
        for heir in heirs:
            ...
            if heir.get("is_minor", False):
                heir_share["note"] = "Người chưa thành niên, cần người đại diện quản lý tài sản"
...
def check_business_name_rules(business_name: str) -> Dict:
    ...
    try:
        ...
        prohibited_words = [
            "việt nam", "quốc gia", "chính phủ", "nhà nước",
            "đảng", "bộ", "ban", "ngành", "ủy ban"
        ]
        
        name_lower = business_name.lower()
        for word in prohibited_words:
            if word in name_lower:
                issues.append(f"Tên không được chứa từ '{word}' (Điều 36 Luật Doanh nghiệp 2020)")
        ...
        if re.search(r'[!@#$%^&*()_+=\[\]{};\'\\:"|,.<>?/~`]', business_name):
            warnings.append("Tên chứa ký tự đặc biệt, có thể gây khó khăn trong đăng ký")
