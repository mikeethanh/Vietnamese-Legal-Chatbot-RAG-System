...
download_data() {
    ...
    python do_spaces_manager.py download-data
...
deploy_docker() {
    ...
    print_success "Service deployed! API available at http://localhost:8000"
    print_status "Check health: curl http://localhost:8000/health"
}
...
upload_model() {
    ...
    python do_spaces_manager.py upload-model "$model_dir"
...
download_model() {
    ...
    python do_spaces_manager.py download-model "$model_name" "$local_dir"
