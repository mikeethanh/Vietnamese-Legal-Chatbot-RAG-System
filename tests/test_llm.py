...
# Suppress Unsloth gradient accumulation warnings (not errors)
import warnings
warnings.filterwarnings("ignore", message=".*num_items_in_batch.*")
warnings.filterwarnings("ignore", message=".*gradient accumulation.*")
...
class FineTuneConfig:
    ...
    max_seq_length: int = 8192  # DOUBLED for H200 - longer legal documents
    dtype: Optional[torch.dtype] = None  # Auto-detect (None for optimization)
    load_in_4bit: bool = False  # H200 has 141GB VRAM - use 16bit for accuracy
    
    # LoRA settings - AGGRESSIVE for Vietnamese legal domain with H200
    lora_r: int = 128  # DOUBLED rank for complex legal reasoning (was 64)
    lora_alpha: int = 256  # 2 * lora_r for optimal learning (128 * 2)
    ...
    use_rslora: bool = True  # Enable for rank stabilization with high rank
    
    # Training settings - AGGRESSIVE for H200 GPU (141GB VRAM) - MAXIMIZE USAGE
    per_device_train_batch_size: int = 32  # INCREASED from 6 to 16
    per_device_eval_batch_size: int = 32  # Match training batch size
    gradient_accumulation_steps: int = 16   # Effective batch size = 16 * 8 = 128 (HUGE!)
    warmup_steps: int = 100  # More warmup for larger batch sizes
    num_train_epochs: int = 4
    max_steps: int = -1  # Use epochs instead
    learning_rate: float = 3e-4  # INCREASED for larger batch (was 2e-4)
    weight_decay: float = 0.01
    lr_scheduler_type: str = "cosine"  # Better for longer training
    seed: int = 3407
    
    # Optimization - AGGRESSIVE settings for H200 maximum utilization
    optim: str = "adamw_torch"  # Full precision optimizer for H200
    fp16: bool = False  # Use bf16 instead on H200
    bf16: bool = True  # H200 supports bfloat16 natively
...
class LlamaFineTuner:
    ...
    def load_model(self):
        ...
        # Load model with Unsloth optimizations - H200 MAXIMIZED
        self.model, self.tokenizer = FastLanguageModel.from_pretrained(
            model_name=self.config.model_name,
            max_seq_length=self.config.max_seq_length,
            dtype=self.config.dtype,  # None for auto-detection
            load_in_4bit=self.config.load_in_4bit,
            # Aggressive memory settings for H200
            attn_implementation="flash_attention_2",  # Flash Attention 2 for efficiency
            device_map="auto",  # Auto device mapping
            # token="hf_..." # Use if model is gated
        )
        
        # Apply LoRA with AGGRESSIVE Unsloth optimizations for H200
        logger.info("🔧 Applying HIGH-RANK LoRA configuration with Unsloth for H200")
        self.model = FastLanguageModel.get_peft_model(
            self.model,
            r=self.config.lora_r,  # HIGH RANK: 128 for complex legal reasoning
            target_modules=["q_proj", "k_proj", "v_proj", "o_proj",
                          "gate_proj", "up_proj", "down_proj",
                          "embed_tokens", "lm_head"],  # Include embedding layers too
            lora_alpha=self.config.lora_alpha,  # 256 for high rank
            ...
            use_rslora=self.config.use_rslora,  # True for rank stabilization
            max_seq_length=self.config.max_seq_length,  # 8192 tokens
        )
        
        # Setup chat template for Llama-3.1
        logger.info("📝 Setting up Llama-3.1 chat template")
        # We'll format the data manually in apply_chat_template_to_datasets method
        # No need to modify tokenizer here
...
{examples['output'][i]}<|eot_id|>"""
    ...
    def setup_trainer(self, datasets: Dict[str, Dataset]):
        ...
        # Training arguments MAXIMALLY optimized for H200 (140GB VRAM)
        training_args = TrainingArguments(
            output_dir=str(self.output_dir),
            run_name=self.config.run_name,
            
            # AGGRESSIVE Batch size and gradient settings for H200
            per_device_train_batch_size=self.config.per_device_train_batch_size,  # 16
            per_device_eval_batch_size=self.config.per_device_eval_batch_size,    # 16
            gradient_accumulation_steps=self.config.gradient_accumulation_steps,  # 8 -> eff_batch=128
            
            # Learning rate and schedule
            learning_rate=self.config.learning_rate,  # 3e-4 for large batch
            weight_decay=self.config.weight_decay,
            lr_scheduler_type=self.config.lr_scheduler_type,
            warmup_steps=self.config.warmup_steps,  # 100 steps
            
            # Training duration
            num_train_epochs=self.config.num_train_epochs,
            max_steps=self.config.max_steps,
            
            # Optimization - H200 MAXIMIZED
            optim=self.config.optim,  # adamw_torch full precision
            fp16=self.config.fp16,    # False
            bf16=self.config.bf16,    # True for H200
            
            # Data loading optimization for H200
        ...
        logger.info(f"   🎯 Effective batch size: {eff_batch} (MASSIVE for H200!)")
        logger.info(f"   📏 Max sequence length: {self.config.max_seq_length} tokens (DOUBLED)")
        logger.info(f"   🔧 LoRA rank: {self.config.lora_r} (HIGH-RANK for complex legal)")
        logger.info(f"   💾 Expected VRAM usage: ~80-120GB (utilizing H200's 140GB)")
        
        # Show evaluation strategy
        if 'val' in datasets:
            logger.info(f"   📈 Validation: Every {training_args.eval_steps if hasattr(training_args, 'eval_steps') else 'epoch'}")
            logger.info(f"   📊 Metrics tracked: train_loss + eval_loss")
